...
public:
	MNPuzzle(int md_ignore_=0);
	MNPuzzle(const std::vector<slideDir> op_order, int md_ignore_=0); // used to set action order
	~MNPuzzle();
...
//	std::vector<std::vector<int> > PDBkey;
	...
	bool use_manhattan;
	bool weighted;
    int md_ignore; // ignores tiles up-to included when calculating heuristic value
...
template <int width, int height>
MNPuzzle<width, height>::MNPuzzle(int md_ignore_)
{
	weighted = false;
	...
	goal_stored = false;
	use_manhattan = true;
    md_ignore = md_ignore_;
}

template <int width, int height>
MNPuzzle<width, height>::MNPuzzle(const std::vector<slideDir> op_order, int md_ignore_)
{
...
//	}
	...
	if (use_manhattan)
	{
		...
		for (unsigned int x = 0; x < width; x++)
		{
			for (unsigned int y = 0; y < height; y++)
			{
				if (state1.puzzle[x + y*width] > md_ignore)
				{
