...
template<int N>
class PancakePuzzle
        : public PermutationPuzzle::PermutationPuzzleEnvironment<PancakePuzzleState<N>, PancakePuzzleAction> {
public:
    ...
    **/
    ...
    static int read_in_pancake_puzzles(const char *filename, bool first_counter, unsigned max_puzzles,
                                       std::vector <PancakePuzzleState<N>> &puzzle_vector);
    ...
    bool
    Path_Check(PancakePuzzleState<N> start, PancakePuzzleState<N> goal, std::vector <PancakePuzzleAction> &actions);
...
template<int N>
PancakePuzzleAction
PancakePuzzle<N>::GetAction(const PancakePuzzleState<N> &parent, const PancakePuzzleState<N> &child) const {
...
//    if (use_memory_free) {
//        //assert(!"This code allocates a vector; re-write to be more efficient");
//        static std::vector<int> state_locs(N);
//        static std::vector<int> goal_locs(N);
//        for (unsigned i = 0; i < N; i++) {
//            state_locs[state.puzzle[i]] = i;
//            goal_locs[goal_state.puzzle[i]] = i;
//        }
//        return std::max(DefaultH(state, goal_locs), DefaultH(goal_state, state_locs));
//    }
...
double symmetric_version(const PancakePuzzleState<N> &state, const std::vector<int> &goal_locs, int gap) {
    ...
    // restore goal
    std::vector<int> restoredGoal(N);
    for (int i = 0; i < N; i++) {
        restoredGoal[goal_locs[i]] = i;
    }

    // create abstract state ignoring pancakes such that < gap
    std::vector<int> abstractState(numberPancakes);
    for (int i = 0, j = 0; i < N; i++)
        if (state.puzzle[i] >= gap)
            abstractState[j++] = state.puzzle[i];

    // same with goal
    std::vector<int> abstractGoal(numberPancakes);
    for (int i = 0, j = 0; i < N; i++)
        if (restoredGoal[i] >= gap)
            abstractGoal[j++] = restoredGoal[i];

    double cost = 0;

    // compute gaps between discs
    for (int i = 0; i < numberPancakes - 1; i++) {
        int upperDisc = abstractGoal[i];
        int lowerDisc = abstractGoal[i + 1];

        int posUpperDisc = std::find(abstractState.begin(), abstractState.end(), upperDisc) - abstractState.begin();
        int posLowerDisc = std::find(abstractState.begin(), abstractState.end(), lowerDisc) - abstractState.begin();

        if (std::abs(posUpperDisc - posLowerDisc) != 1)
            cost++;
    }

    // last disc must be the same one
    if (abstractState[numberPancakes - 1] != abstractGoal[numberPancakes - 1])
        cost++;

    // return symmetric version by default
    return cost;
...
template<int N>
void PancakePuzzle<N>::Create_Random_Pancake_Puzzles(std::vector <PancakePuzzleState<N>> &puzzle_vector,
    ...
    while (count < num_puzzles) {
        perm = PermutationPuzzle::PermutationPuzzleEnvironment<PancakePuzzleState<N>, PancakePuzzleAction>::Get_Random_Permutation(
                N);
...
template<int N>
int PancakePuzzle<N>::read_in_pancake_puzzles(const char *filename, bool puzz_num_start, unsigned max_puzzles,
                                              std::vector <PancakePuzzleState<N>> &puzzles) {

    std::vector <std::vector<int>> permutations;
    PermutationPuzzle::PermutationPuzzleEnvironment<PancakePuzzleState<N>, PancakePuzzleAction>::Read_In_Permutations(
            filename, N, max_puzzles, permutations, puzz_num_start);
...
template<int N>
bool PancakePuzzle<N>::Path_Check(PancakePuzzleState<N> start, PancakePuzzleState<N> theGoal,
