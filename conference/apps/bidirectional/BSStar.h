...
template<class state, class action, class environment, class priorityQueue>
bool BSStar<state, action, environment, priorityQueue>::InitializeSearch(environment *env, const state &from,
                                                                         const state &to,
...
void BSStar<state, action, environment, priorityQueue>::Trim() {
    for (unsigned int x = 0; x < forwardQueue.OpenSize(); x++) {
        ...
        if (data.where == kOpenList && fgreatereq(data.g + data.h, currentCost) &&
            (data.data != middleNode)) // and not start or goal
        {
