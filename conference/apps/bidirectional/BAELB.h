...
class BAELB {
public:
    BAELB(int forwardMainHeapNum, int backwardMainHeapNum,
          bool alternating_ = true, double epsilon_ = 1.0, double gcd_ = 1.0, bool withLog_=true) {
        forwardHeuristic = 0;
        backwardHeuristic = 0;
        env = 0;
        ResetNodeCount();
    ...
    BAELB(int mainHeapNum, bool alternating_ = true, double epsilon_ = 1.0, double gcd_ = 1.0, bool withLog_ = true) :
            BAELB(mainHeapNum, mainHeapNum, alternating_, epsilon_, gcd_, withLog_) {}
...
private:
    ...
    void Expand(priorityQueue &current, priorityQueue &opposite,
                Heuristic<state> *heuristic, Heuristic<state> *reverse_heuristic,
    ...
    for (auto &succ: neighbors) {
        ...
        switch (loc) {
            ...
            case kNotFound: { //New Node
                ...
                current.AddOpenNode(succ, hash, g, h, reverse_heuristic->HCost(succ, source), nextID);
                // Check for solution
                uint64_t reverseLoc;
                if (opposite.Lookup(hash, reverseLoc) == kOpenList) {
