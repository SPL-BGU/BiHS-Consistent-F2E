...
public:
    ...
    inline const DVCBSOpenClosedData<state> &GetForwardItem(unsigned int which) {
        return queue.forwardQueue.Lookat(which);
    }

    inline const int GetNumBackwardItems() { return queue.backwardQueue.size(); }

    inline const DVCBSOpenClosedData<state> &GetBackwardItem(unsigned int which) {
        return queue.backwardQueue.Lookat(which);
    }
...
private:
    ...
    double ExtractCostToGoal(state &node) {
        uint64_t theID;
        queue.backwardQueue.Lookup(env->GetStateHash(node), theID);
        return ExtractCostToGoalFromID(theID);
    }
    ...
    double ExtractCostToStart(state &node) {
        uint64_t theID;
        queue.forwardQueue.Lookup(env->GetStateHash(node), theID);
        return ExtractCostToStartFromID(theID);
    }
...
void DVCBS<state, action, environment, dataStructure, priorityQueue>::GetPath(environment *env, const state &from,
                                                                              const state &to,
...
template<class state, class action, class environment, class dataStructure, class priorityQueue>
bool
DVCBS<state, action, environment, dataStructure, priorityQueue>::InitializeSearch(environment *env, const state &from,
                                                                                  const state &to,
                                                                                  Heuristic <state> *forward,
                                                                                  Heuristic <state> *backward,
                                                                                  std::vector <state> &thePath) {
...
bool DVCBS<state, action, environment, dataStructure, priorityQueue>::ExpandAVertexCover(std::vector <state> &thePath) {
    ...
    if ((!isAllSolutions && !fless(queue.GetLowerBound(), currentCost)) ||
        (isAllSolutions && !flesseq(queue.GetLowerBound(), currentCost))) {
        ExtractFromMiddle(thePath);
        return true;
...
void DVCBS<state, action, environment, dataStructure, priorityQueue>::Expand(uint64_t nextID,
    ...
    for (auto &succ : neighbors) {
        ...
        if ((!isAllSolutions && fgreatereq(current.Lookup(nextID).g + edgeCost, currentCost)) ||
            (isAllSolutions && fgreater(current.Lookup(nextID).g + edgeCost, currentCost))) {
            continue;
        }
        ...
        switch (loc) {
            ...
            case kUnseen: {
                ...
                } else//loc == kUnseen
                {
                    ...
                    if ((!isAllSolutions && fless(newNodeF, currentCost)) ||
                        (isAllSolutions && flesseq(newNodeF, currentCost))) {

                        if (fless(newNodeF, queue.GetLowerBound()) ||
                            (isLEQ && flesseq(newNodeF, queue.GetLowerBound())))
