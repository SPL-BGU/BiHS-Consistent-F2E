#ifndef APPS_AIJ_DBBSLB_H
#define APPS_AIJ_DBBSLB_H
...
bool DBBSLB<state, action, environment, criterion, priorityQueue>::UpdateC() {
    ...
    while (C < currentCost && (!forwardQueue.isBestBucketComputed() || !backwardQueue.isBestBucketComputed())) {
        ...
        forwardQueue.computeBestBucket(C, C, C, 2.0 * C, DBL_MAX, DBL_MAX, 2.0 * C, 2.0 * C, 3.0 * C, DBL_MAX, DBL_MAX,
        ...
        while (limitsChanged) {
            ...
            backwardQueue.computeBestBucket(C - (gMinF + epsilon), C - dMinF, C - fMinF,
                                            ...
                                            2.0 * C - (DGMinF + epsilon), 2.0 * C - (FGMinF + epsilon),
                                            3.0 * C - (BGMinF + epsilon),
                                            2.0 * C - RFDMinF, 2.0 * C - FRDMinF, 2.0 * C - (RDGMinF + epsilon),
                                            2.0 * C - (RFGMinF + epsilon),
                                            2.0 * C - RFRDMinF, 3.0 * C - (RFDGMinF + epsilon),
                                            3.0 * C - (FRDGMinF + epsilon), 3.0 * C - (RFRDGMinF + epsilon)
            );
            ...
            forwardQueue.computeBestBucket(C - (gMinB + epsilon), C - dMinB, C - fMinB,
                                           ...
                                           2.0 * C - (DGMinB + epsilon), 2.0 * C - (FGMinB + epsilon),
                                           3.0 * C - (BGMinB + epsilon),
                                           2.0 * C - RFDMinB, 2.0 * C - FRDMinB, 2.0 * C - (RDGMinB + epsilon),
                                           2.0 * C - (RFGMinB + epsilon),
                                           2.0 * C - RFRDMinB, 3.0 * C - (RFDGMinB + epsilon),
                                           3.0 * C - (FRDGMinB + epsilon), 3.0 * C - (RFRDGMinB + epsilon)
            );
            ...
            limitsChanged = gMinF != gMinF_new || fMinF != fMinF_new || dMinF != dMinF_new
                            ...
                            || FGMinF != FGMinF_new || DGMinF != DGMinF_new || BGMinF != BGMinF_new ||
                            FRDMinF != FRDMinF_new
                            || RFDMinF != RFDMinF_new || RFGMinF != RFGMinF_new || RDGMinF != RDGMinF_new ||
                            RFRDMinF != RFRDMinF_new
...
//            C += gcd;
    ...
    if (side_crit == SideCriterion::Cardinality && forwardQueue.isBestBucketComputed() &&
        backwardQueue.isBestBucketComputed()) {
...
double DBBSLB<state, action, environment, criterion, priorityQueue>::GetNextC() {
    ...
    double total_g_bound = DBL_MAX;
    double total_fd_bound = DBL_MAX;
    double total_df_bound = DBL_MAX;
    double total_b_bound = DBL_MAX;
    double total_rfrd_bound = DBL_MAX;
    double total_rdrf_bound = DBL_MAX;
    // fgdg bound
    double total_fgdg_bound = DBL_MAX;
    // dgfg bound
    double total_dgfg_bound = DBL_MAX;
    // bgbg bound
    double total_bgbg_bound = DBL_MAX;
    // frdrfd bound
    double total_frdrfd_bound = DBL_MAX;
    // rfdfrd bound
    double total_rfdfrd_bound = DBL_MAX;
    // rfgrdg bound
    double total_rfgrdg_bound = DBL_MAX;
    // rdgrfg bound
    double total_rdgrfg_bound = DBL_MAX;
    // rfrdrfrd bound
    double total_rfrdrfrd_bound = DBL_MAX;
    // frdgrfdg bound
    double total_frdgrfdg_bound = DBL_MAX;
    // rfdgfrdg bound
    double total_rfdgfrdg_bound = DBL_MAX;
    // rfrdgrfrdg bound
    double total_rfrdgrfrdg_bound = DBL_MAX;
    ...
    for (const double fw_g_value: forward_g_values)
        for (const double bw_g_value: backward_g_values) {
            ...
            if (g_bound > C && g_bound < result)
                result = g_bound;
            if (g_bound < total_g_bound) {
                total_g_bound = g_bound;
            }
        }
    ...
    for (const double fw_f_value: forward_f_values)
        for (const double bw_d_value: backward_d_values) {
            ...
            if (fw_KK_bound > C && fw_KK_bound < result)
                result = fw_KK_bound;
            if (fw_KK_bound < total_fd_bound) {
                total_fd_bound = fw_KK_bound;
            }
        }
    ...
    for (const double bw_f_value: backward_f_values)
        for (const double fw_d_value: forward_d_values) {
            ...
            if (bw_KK_bound > C && bw_KK_bound < result)
                result = bw_KK_bound;
            if (bw_KK_bound < total_df_bound) {
                total_df_bound = bw_KK_bound;
            }
        }
    ...
    for (const double fw_rf_value: forward_rf_values)
        for (const double bw_rd_value: backward_rd_values) {
            ...
            if (fw_RC_bound > C && fw_RC_bound < result)
                result = fw_RC_bound;
            if (fw_RC_bound < total_rfrd_bound) {
                total_rfrd_bound = fw_RC_bound;
            }
        }
    ...
    for (const double bw_rf_value: backward_rf_values)
        for (const double fw_rd_value: forward_rd_values) {
            ...
            if (bw_RC_bound > C && bw_RC_bound < result)
                result = bw_RC_bound;
            if (bw_RC_bound < total_rdrf_bound) {
                total_rdrf_bound = bw_RC_bound;
            }
        }
    ...
    for (const double fw_fg_value: forward_fg_values)
        for (const double bw_dg_value: backward_dg_values) {
            ...
            if (fgdg_bound > C && fgdg_bound < result)
                result = fgdg_bound;
            if (fgdg_bound < total_fgdg_bound) {
                total_fgdg_bound = fgdg_bound;
            }
        }
    ...
    for (const double fw_dg_value: forward_dg_values)
        for (const double bw_fg_value: backward_fg_values) {
            ...
            if (dgfg_bound > C && dgfg_bound < result)
                result = dgfg_bound;
            if (dgfg_bound < total_dgfg_bound) {
                total_dgfg_bound = dgfg_bound;
            }
        }
    ...
    for (const double fw_bg_value: forward_bg_values)
        for (const double bw_bg_value: backward_bg_values) {
            ...
            if (bgbg_bound > C && bgbg_bound < result)
                result = bgbg_bound;
            if (bgbg_bound < total_bgbg_bound) {
                total_bgbg_bound = bgbg_bound;
            }
        }
    ...
    for (const double fw_frd_value: forward_frd_values)
        for (const double bw_rfd_value: backward_rfd_values) {
            ...
            if (frdrfd_bound > C && frdrfd_bound < result)
                result = frdrfd_bound;
            if (frdrfd_bound < total_frdrfd_bound) {
                total_frdrfd_bound = frdrfd_bound;
            }
        }
    ...
    for (const double fw_rfd_value: forward_rfd_values)
        for (const double bw_frd_value: backward_frd_values) {
            ...
            if (rfdfrd_bound > C && rfdfrd_bound < result)
                result = rfdfrd_bound;
            if (rfdfrd_bound < total_rfdfrd_bound) {
                total_rfdfrd_bound = rfdfrd_bound;
            }
        }
    ...
    for (const double fw_rfg_value: forward_rfg_values)
        for (const double bw_rdg_value: backward_rdg_values) {
            ...
            if (rfgrdg_bound > C && rfgrdg_bound < result)
                result = rfgrdg_bound;
            if (rfgrdg_bound < total_rfgrdg_bound) {
                total_rfgrdg_bound = rfgrdg_bound;
            }
        }
    ...
    for (const double fw_rdg_value: forward_rdg_values)
        for (const double bw_rfg_value: backward_rfg_values) {
            ...
            if (rdgrfg_bound > C && rdgrfg_bound < result)
                result = rdgrfg_bound;
            if (rdgrfg_bound < total_rdgrfg_bound) {
                total_rdgrfg_bound = rdgrfg_bound;
            }
        }
    ...
    for (const double fw_rfrd_value: forward_rfrd_values)
        for (const double bw_rfrd_value: backward_rfrd_values) {
            ...
            if (rfrdrfrd_bound < total_rfrdrfrd_bound) {
                total_rfrdrfrd_bound = rfrdrfrd_bound;
            }
        }
    ...
    for (const double fw_frdg_value: forward_frdg_values)
        for (const double bw_rfdg_value: backward_rfdg_values) {
            ...
            if (frdgrfdg_bound < total_frdgrfdg_bound) {
                total_frdgrfdg_bound = frdgrfdg_bound;
            }
        }
    ...
    for (const double fw_rfdg_value: forward_rfdg_values)
        for (const double bw_frdg_value: backward_frdg_values) {
            ...
            if (rfdgfrdg_bound < total_rfdgfrdg_bound) {
                total_rfdgfrdg_bound = rfdgfrdg_bound;
            }
        }
    ...
    for (const double fw_rfrdg_value: forward_rfrdg_values)
        for (const double bw_rfrdg_value: backward_rfrdg_values) {
            ...
            if (rfrdgrfrdg_bound < total_rfrdgrfrdg_bound) {
                total_rfrdgrfrdg_bound = rfrdgrfrdg_bound;
            }
        }
    int max_idx = arg_max<double>({total_b_bound, total_g_bound, total_fd_bound, total_df_bound, total_rfrd_bound,
                                   total_rdrf_bound, total_fgdg_bound, total_dgfg_bound, total_bgbg_bound,
                                   total_frdrfd_bound, total_rfdfrd_bound, total_rfgrdg_bound, total_rdgrfg_bound,
                                   total_rfrdrfrd_bound, total_frdgrfdg_bound, total_rfdgfrdg_bound,
                                   total_rfrdgrfrdg_bound});
    switch (max_idx) {
        case 1:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinG);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinG);
            break;
        case 2:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinF);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinD);
            break;
        case 3:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinF);
            break;
        case 0:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinB);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinB);
            break;
        case 4:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRF);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRD);
            break;
        case 5:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRF);
            break;
        case 6:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGF);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGD);
            break;
        case 7:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGF);
            break;
        case 8:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGB);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGB);
            break;
        case 9:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinFRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRFD);
            break;
        case 10:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRFD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinFRD);
            break;
        case 11:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRF);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRD);
            break;
        case 12:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRF);
            break;
        case 13:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRFRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinRFRD);
            break;
        case 14:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGFRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRFD);
            break;
        case 15:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRFD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGFRD);
            break;
        case 16:
            forwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRFRD);
            backwardQueue.setCurrentMaxTotExpansion(MinCriterion::MinGRFRD);
            break;
        default:
            throw std::out_of_range("Invalid bound id");
            break;
    }
    return result;
}
