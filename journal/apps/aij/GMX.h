...
public:
    ...
    void GenerateBuckets(std::vector<AStarOpenClosedDataWithF<state>> &nodes, Heuristic<state> *r_h,
                         state hstate, double solution_cost_, bool forward);

    int GetMinimalExpansions();

    inline double GetSolutionCost() {
...
template<class state>
void GMX<state>::GenerateBuckets(std::vector<AStarOpenClosedDataWithF<state>> &nodes, Heuristic<state> *r_h,
                                 state hstate, double solution_cost_, bool forward) {
